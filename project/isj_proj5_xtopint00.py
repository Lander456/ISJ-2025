...
def strs_for_number(number: int, digit2chars: dict[int,str]) -> list[str]:
    ...
    for digit in str(number):
        ...
        letters = digit2chars[int(digit)]

        str_components_to_product.append(letters)
...
def words_with_same_number(words: list[str],
    ...
    for key in digit2chars.keys():

        for letter in digit2chars[key]:

            char2digit_as_str[str(letter)] = int(key)
    ...
    for w in words:
        ...
        for letter in w:
            ...
            numbedWord.append(str(char2digit_as_str[letter]))

        numbedWord = ''.join(numbedWord)

        numb2words[numbedWord].append(w)
    ...
    for key in numb2words.keys():

        if len(numb2words[key]) > 1:

            result[int(key)] = numb2words[key]
